...
wav_path = "/home/pi/PBL3/alarm.wav"
...
# Khoi tao bien neu bi canh bao qua so lan quy dinh se gui email cho canh sat
alertNum = 0
alertNumPolice = 10 
	...
	for (x, y, w, h) in faces:
		...
		if eye_avg_ratio < eye_ratio_threshold:
			...
			if sleep_frames >= max_sleep_frames:
				...
				cv2.putText(frame, "SYSTEM ALERT!!!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
				
				# Gui email cho cong an neu bi canh bao qua so lan quy dinh
				alertNum += 1
				if alertNum > alertNumPolice:
